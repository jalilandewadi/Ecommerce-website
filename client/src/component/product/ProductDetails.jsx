...
const ProductDetails = ({ productId }) => {
  ...
  useEffect(() => {
    if (productFecthId) {
      dispatch(fetchProductsDetails(productFecthId));
      dispatch(fetchSimilarProducts({ id: productFecthId }));
    }
                  ...
                  {selectedProducts?.colors?.map((color) => (
                    <button
                      ...
                      className={`w-8 h-8 rounded-full border ${
                        selectedColor === color
                          ? "border-3 border-r-black"
                          : "border-gray-400"
                      }`}
                      style={{
                        backgroundColor: color.toLocaleLowerCase(),
                        // Note: The original image showed an incomplete 'filter' property.
                        // It has been completed to a sensible default.
